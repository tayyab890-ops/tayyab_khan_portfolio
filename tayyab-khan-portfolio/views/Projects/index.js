...
const ProjectTextSide = (props) => {
    const { label, title, description, techs, index, image, github, live } = props
    const side = getSide(index);
    return (
        <div
            ...
            <div
                data-aos={`zoom-in-${side}`}
                className='ai-projects-tech-badges'>
                {techs.map((tech, i) => (
                    <span key={i} className='ai-tech-badge'>{tech}</span>
                ))}
            </div>
            <div data-aos={`zoom-in-${side}`} className='ai-projects-links'>
                {github && (
                    <a href={github} target='_blank' rel='noreferrer' className='ai-button-sm ai-project-link'>
                        GitHub
                    </a>
                )}
                {live && (
                    <a href={live} target='_blank' rel='noreferrer' className='ai-button-sm ai-project-link'>
                        Live Demo
                    </a>
                )}
            </div>
